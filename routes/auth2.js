...
router.post("/login", async (req, res) => {
    ...
    res.status(201).json({
      ...
      user: {
        firstName: profile? profile.given_name: email,
        lastName: profile?profile.family_name: email,
        picture: profile? profile.picture:"",
        usernameOrEmail: profile? profile.email: email,
        token: jwt.sign({ email: email }, process.env.JWT_SECRET || 'mysecretkey', {
          expiresIn: "1d",
        }),
      },
    });
